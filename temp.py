...
class BatchDataset(Dataset):
    def __init__(self, dataset, seq_length) -> None:
        ...
        self.idx = list(range(0, self.S, seq_length))ㅉㅁ
...
class VIGGODataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 토크나이징
        input_encoded = self.tokenizer(input_text, padding="max_length", truncation=True, max_length=self.max_length, return_tensors="pt")
        target_encoded = self.tokenizer(target_text, padding="max_length", truncation=True, max_length=self.max_length, return_tensors="pt")
        ...
        # 손실 계산을 위해 padding 토큰은 -100으로 설정
